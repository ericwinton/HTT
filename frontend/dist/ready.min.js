var ReadyJS = function(options) {
	...
	var getRouteData = async (callback) => {
		if (this.routeData[this.route.component]) {
			await this.routeData[this.route.component]();
		}
	...
	this.render = (componentName, props = {}, target = null, domAction = 'replace') => {
		...
		if (!this.components[cName]) { console.error('ReadyJS Error: Component not found: "' + cName + '"'); return; }
		...
		if (c.template) {
			...
			if (!templateBody.children.length) { console.error('ReadyJS Error: The "' + cName + '" component template does not have a root element'); return; }
			...
			if (templateDom) {
				...
				if (target) {					
					...
					console.log('returning ', templateBody.innerHTML);
